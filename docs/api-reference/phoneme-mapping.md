...
- 音素列への BOS/EOS/パディング付与は `piper_plus_g2p/encode/encoder.py` の `PiperEncoder` クラス内部で処理される
...
固定割り当ては U+E000〜U+E061 の範囲で、U+E062 以降は動的割り当て用に予約されている。
...
| トークン | PUA | 説明 |
|---------|-----|------|
| `iː` | U+E059 | 非円唇前舌狭長母音 (例: vi, liten) |
| `yː` | U+E05A | 円唇前舌狭長母音 (例: ny, syster) |
| `eː` | U+E05B | 非円唇前舌半狭長母音 (例: se, leva) |
| `ɛː` | U+E05C | 非円唇前舌半広長母音 (例: häl, läsa — ä の長音) |
| `øː` | U+E05D | 円唇前舌半狭長母音 (例: öl, söka — ö の長音) |
| `ɑː` | U+E05E | 非円唇後舌広長母音 (例: far, tala — a の長音) |
| `oː` | U+E05F | 円唇後舌半狭長母音 (例: mor, lov — å の長音, o 例外語) |
| `uː` | U+E060 | 円唇後舌狭長母音 (例: o のデフォルト長音, hus) |
