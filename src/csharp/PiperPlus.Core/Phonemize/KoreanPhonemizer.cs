...
/// <summary>
/// Korean phonemizer that wraps an <see cref="IKoreanG2PEngine"/>.
/// <para>
/// The engine already returns PUA-mapped tokens and per-token prosody
/// (A1=0, A2=0, A3=Hangul syllable count per word), so this class is
/// a thin pass-through that validates array consistency and delegates
...
/// </para>
/// </summary>
public sealed class KoreanPhonemizer : IPhonemizer
{
    ...
    /// </summary>
    /// <param name="engine">
    /// Korean G2P engine that produces PUA-mapped phonemes with prosody.
    /// </param>
    public KoreanPhonemizer(IKoreanG2PEngine engine)
    {
    ...
    /// <inheritdoc />
    /// <remarks>
    /// Returns <c>null</c> --- Korean models use the phoneme-ID map from config.json.
    /// </remarks>
    public Dictionary<string, int[]>? GetPhonemeIdMap() => null;

    /// <inheritdoc />
    /// <remarks>
    /// Korean requires inter-phoneme padding and BOS/EOS wrapping
