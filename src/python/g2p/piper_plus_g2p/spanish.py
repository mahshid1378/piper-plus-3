...
def _has_accent(word: str) -> int | None:
    ...
    Only stress-indicating accents are considered.
    The diaeresis changes pronunciation but does NOT affect stress.
    """
...
def _segment_graphemes(word: str) -> list[_GraphemeUnit]:
    ...
    ``gu``, ``sc`` before e/i) are kept as single units so that
    ...
    while i < n:
        ...
        # ``gu`` before e/i -- diaeresis makes the u pronounced (/gw/)
        if (
            bch == "g"
            and i + 1 < n
            and word[i + 1] == "\u00fc"
...
def _find_syllable_boundaries(
    ...
    *,
    ...
    digraphs (``ch``, ``ll``, ``rr``, ``qu``, ``gu``) are treated
...
def _insert_stress_marker(
    ...
    *,
    ...
    """Insert stress marker before the stressed syllable's first vowel.

    Uses ``_segment_graphemes`` for reliable char-to-phoneme mapping so
    that digraphs like ``qu``, ``gu`` are handled correctly.
    """
    if not phonemes:
        return phonemes
